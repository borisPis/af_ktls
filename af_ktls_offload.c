...
int tls_sendmsg_with_offload(struct tls_sock *tsk, struct msghdr *msg, size_t size) {
...
static struct net_device *get_netdevice_for_socket(struct sock *sk) {
        /* [SR] TODO: IPv6 support? What happens if the socket is not
         * inet socket? Should fail gracefully. Ditto if the socket is
         * not connected.
         * [BP]: Actually the struct is the same for both ipv4 and ipv6.
         * (querying the struct is done using the same code)
         * I don't think the socket can be other than an inet_sock when we
         * reach this function. Also, the tcp state is checked before calling
         * this function - as you can see by the comment above.
         */
...
static int attach_sock_to_netdev(struct sock *sk, struct ktls_keys *keys)
{
	...
	if (!netdev->ktls_ops) {
		pr_err("attach_sock_to_netdev: netdev %s does not support TLS offload\n", netdev->name);
		goto out;
	}

	if (netdev->ktls_ops->ktls_dev_add(netdev, sk, keys)) {
	...
	/* [I.L] TODO: I think we should
	 * hold a reference to the netdev and
	 * if it goes down encrypt TCP data
	 * and only then release the reference
	 */
out:
	dev_put(netdev);
	return ret;
...
int tls_set_offload(struct socket *sock, char __user *src, size_t src_len)
{
	...
	if (attach_sock_to_netdev(offloaded_sock->sk, &keys))
		return -EINVAL;


	offloaded_sock->sk->sk_offloaded = 1;
	context = offloaded_sock->sk->sk_tls_offload;
	...
	inet_csk(offloaded_sock->sk)->icsk_offload_ops = &tls_offload_ops;
